...
import { fetchDiscoverTvSeries, fetchMedia, fetchOnlyTvSeries, fetchTrendingMedia } from "../../services/MediaService";
...
const MultiSearchPage: React.FC = () => {
  ...
  if(medias.length === 0){
    fetchTrendingMedia().then(setMedias).catch((err) => console.error(err));
  }
